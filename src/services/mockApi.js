...
const MockApiService = {
  ...
  sendWhatsAppNotification: async (bill, billId) => {
    ...
    try {
      ...
      // Generate the bill message with improved formatting
      let messageContent = `*📋 BILL NOTIFICATION*\n\nDear ${
        bill.customerName
      },\n\nYour bill from *Mammta's Food* is ready!\n\n📌 *Bill #${billId}*\n📅 Date: ${new Date(
        bill.date
      ).toLocaleDateString()}\n`;
      ...
      if (bill.itemsDetail && bill.itemsDetail.length > 0) {
        messageContent += `🛍️ *Items:*\n${bill.itemsDetail
          .split(", ")
          .join("\n")}\n`;
      } else {
        messageContent += `🛍️ *Items:* ${bill.items} items\n`;
      }

      // Add total amount
      messageContent += `💰 *Total Amount:* ₹${bill.total.toFixed(2)}\n\n`;

      // Add payment status based on the bill status
      if (bill.status === "pending") {
        messageContent += `⏳ *Payment Status:* PAYMENT PENDING\n\n`;
        messageContent += `Your payment is pending.\n\n💳 *Pay via UPI:* 9309908454@ybl\n\n📱 We'll send payment updates via WhatsApp`;
      } else if (bill.status === "paid") {
        messageContent += `✅ *Payment Status:* PAYMENT COMPLETED\n\n`;
        messageContent += `Thank you for your payment!\n\n🧾 *View receipt:* ${receiptLink}\n\n*Important:* Save this message to access your receipt anytime.`;
      } else if (bill.status === "cancelled") {
        messageContent += `❌ *Payment Status:* CANCELLED\n\n`;
        messageContent += `This bill has been cancelled. Please contact us if you have any questions.`;
      }
      ...
      if (typeof window !== "undefined") {
        ...
        return {
          ...
          recipient: bill.phone,
          billId: billId,
          receiptUrl: bill.status === "paid" ? receiptLink : null,
        };
      } else {
